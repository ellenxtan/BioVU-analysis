...
FUN_rhee_infection_new = function(cam_visits) {
  ...
  ## blood culture day within 1 day of hospital admission date 
  blood_raw2 <- sqldf::sqldf('SELECT * 
                           FROM cam_visits as t1
                           INNER JOIN blood_raw1 as t2
                           ON t1.grid = t2.grid AND blood_date <= dc_date AND blood_date BETWEEN icu_start_date-1 AND icu_start_date+1') %>%
    setNames(make.unique(names(.))) %>%  # make unique column names
    as_tibble() %>% 
    select(grid, adm_date, dc_date, icu_start_date, blood_date) %>% 
    mutate(day = as.numeric(blood_date - icu_start_date) + 1) %>%   # xtan: from blood_date to icu_start_date
...
# write_csv(rhee_infection, "../datafile/rhee_infection_new_20200609.csv")
